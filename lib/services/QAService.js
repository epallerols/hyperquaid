...
module.exports = {
  ...
  getQaidByQuestionId: function getQaid(questionId, callback) {
    QA.find({
  ...
  createAnswer: function createAnswer(questionId, data, callback) {
    return this.getQaidByQuestionId(questionId, function updateQaid(error, qaModel) {
      var index;
      ...
      index = qaModel.questions.findIndex(function findQuestion(
        question) {
        return question._id && question._id.toString() ===
          questionId;
      });

      if (!qaModel.questions[index].answers || !Array.isArray(qaModel.questions[
          index].answers)) {
